import { ContainerHiddenRow } from "../widgets/components";

export default function Contacts() {
  return (
    <div className="flex items-center w-auto h-auto justify-center bg-(--bgc) text-(--text)">
      <main className="flex w-full h-full flex-row items-center justify-center">
        <ContainerHiddenRow isFitted={true} isCentered={true}>
          ...
          <div className="flex flex-col items-start">
            <h1 className="text-2xl  flex gap-3">Username: <span className="font-bold text-(--mode)">misterD05</span></h1>
            <a
                    href={`https://github.com/misterD05`}
                    className={
                        'px-5 py-2.5 bg-(--bg) hover:bg-(--text)/20 border border-(--text) text-(--text) font-medium rounded-xl transition-all flex items-center gap-2 w-fit mt-2'
                    }
                >
